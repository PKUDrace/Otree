...
//前被试插入
function beforeInsert(uId,gId,uData) {
    // 构造数据对象
    const dataToSend = {
        uId,
        gId,
    ...
    fetch("https://decisionno1.netlify.app/.netlify/functions/BeforeInsert", {
        method: "POST",
        headers: {
...
function BeforeFind() {
    ...
    .then(data => {
        console.log("成功接收响应:", data);
        if (data.success) {
            console.log("数据查询成功:", data.documents);
            // 处理返回的数据，例如显示在页面上
