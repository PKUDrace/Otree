...
const videoIntro2 = {
    ...
    stimulus: ['img/game2_intro.mp4'],
    prompt: '<div style="position: absolute; top: 20px; left: 50%; transform: translateX(-50%); z-index: 1000;"><h2 style="text-align: center; margin: 0; background-color: rgba(255,255,255,0.9); padding: 10px; border-radius: 5px;">游戏2介绍视频</h2></div>',
    width: 800,
    height: 600,
    autoplay: true,
    controls: true,
    ...
    button_html: '<button class="jspsych-btn" style="margin: 20px 10px; background-color: rgb(75, 126, 243); color: white; border: none; border-radius: 5px; padding: 10px 20px; cursor: pointer;">%choice%</button>',
    on_finish: function(data) {
        if (data.response === 0) {
            data.replay_video = true;
        } else {
            data.replay_video = false;
        }
    }
};
//视频循环播放
const videoLoop2 = {
    timeline: [videoIntro2],
    loop_function: function() {
        const lastData = jsPsych.data.get().last(1).values()[0];
        return lastData.replay_video === true; // 如果选择重新播放，继续循环
    }
};
// 介绍页
const intro_game2 = {
        ...
        <div style="text-align: left; margin: 50px 150px;">
            ...
            <div style="display: flex; gap: 40px; align-items: flex-start;">
                <div style="flex: 1;">
                    ...
                    <p>注意：收益只跟你和对手<span style="font-weight: bold;color: rgb(142,27,17);">第几轮做判断以及判断的对错有关</span>，而<span style="font-weight: bold;color: rgb(142,27,17);">与鼠标选择选项的快慢无关！</span></p>
                </div>
...
let game2_end = {
    ...
    stimulus: `
      ...
      <h3 style="margin-bottom:30px">上传数据中，请稍等片刻，切勿刷新或关闭网页...</h3>
      `,
